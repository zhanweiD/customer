        ...
        data.map(item => {
          return (
            <div className="dfsa item-content">
              ...
              <div 
                className="FBH w100 FBJA" 
                // onClick={() => window.location.href = `${window.__keeper.pathHrefPrefix}/customer/portrait/${item.ident}/${id}`}
              >
                {/* <div>{`${item.客户姓名} ${item.type}`}</div> */}
                {/* <div>{item.客户姓名}</div> */}
                <Link target="_blank" to={`/customer/portrait/${item.ident}/${id}`}>{`${item.客户姓名}`}</Link>
                <div className="c65">{item.手机号}</div>
                <div className="c65">{item.recentTime}</div>
              </div>
