...
const ChannelManage = () => {
  ...
  const authoriza = async name => {
    ...
    if (name === '微信公众号') {
      ...
      window.open(`http://zdhyx.dc.dtwave.com/customer/index.html#/weappCode/${localStorage.getItem('userAccount')}/${host[2]}/${1}`)
    }
    if (name === '微信小程序') {
      const host = window.location.href.split('/')
      window.open(`http://zdhyx.dc.dtwave.com/customer/index.html#/weappCode/${localStorage.getItem('userAccount')}/${host[2]}/${2}`)
    }
    if (name === '阿里云短信') {
      setDrawer(true)
    }
